		...
		<div class="priview-memorandum">
			<div class="row">
				<div class="col-12 text-center">
					...
					<?= !empty($finance_info)?'অর্থায়নেঃ '.$finance_info->finance_name.'<br>':''?>
					<?=date_bangla_calender_format($this->input->post('start_date'))?> <em>হতে</em> <?=date_bangla_calender_format($this->input->post('end_date'))?> <em> তারিখ পর্যন্ত </em>
				</div>
			</div>
		</div>
			...
			<table class="table table-hover table-bordered report">
				<thead class="headding">
					<tr>
						...
						<th class="text-center">প্রশিক্ষণ কোর্সের নাম</th>
						<th class="text-center">অংশগ্রহণকারী</th>
						<th class="text-center">কোর্স শুরু ও শেষের তারিখ	</th>
						<th class="text-center">কোর্সের মেয়াদ (দিন)</th>
						<!-- <th class="text-center">অংশগ্রহণকারীর সংখ্যা</th> -->
						<th class="text-center">অর্থায়নে</th>
				...
				<tbody>
					...
					foreach ($results as $row) { 
						$i++;
//Day count
$training_start = $row->start_date != '0000-00-00' ? $row->start_date:'';
$training_end = $row->end_date != '0000-00-00' ? $row->end_date:'';
...
$duration = eng2bng($duration).' দিন';
						...
						<tr>
							...
							<td class="text-left"><?=$row->course_title?></td>
							<td class="text-left"><?=$row->participant_name?></td>
							<td class="text-left"><?=date_bangla_calender_format($row->start_date)?> হতে <?=date_bangla_calender_format($row->end_date)?></td>
							<td class="text-center"><?=$duration?></td>
							<td class="text-left"><?=$row->finance_name?></td>
							<td>&nbsp;</td>		
						</tr>
						<?php } ?>
					</tbody>
