			...
			<table class="table table-hover table-bordered report">
				<thead class="headding">
					<tr>
						...
						<td class="text-left">উপজেলার নাম</td>												
				...
				<tbody>
					<?php 
					$subtotal=$grandTotal_upazila_p=$grandTotal_pourasava=$grandTotal_union_p=$grandTotal=0;
					foreach ($result_data as $key => $row) { 
						// dd($row->dis_name_bn);
						
						$upazila_p = $row->count[0]->upazila_p?$row->count[0]->upazila_p:0;
						$pourasava = $row->count[0]->pourasava?$row->count[0]->pourasava:0;
						$union_p = $row->count[0]->union_p?$row->count[0]->union_p:0;

						$subtotal = $upazila_p+$pourasava+$union_p;
