...
class Exam_names extends MX_Controller {
	
    function __construct() {
        parent::__construct();
        $this->load->library('form_validation');
        $this->load->model('Exam_names_model');
    ...
    public function add() {
		...
		$data['status']=array('id','name',array(
								'0'=>array('id'=>'Active','name'=>lang('Active')),
								'1'=>array('id'=>'Inactive','name'=>lang('Inactive'))
							));
		
		$children=array();
		for($i=0;$i<=10;$i++)
			$children[]=array('id'=>$i,'name'=>$i);			
		$data['daughter_number']=$data['son_number']=array('id','name',$children);
        ...
        if ($this->form_validation->run() === false) {
			...
			// $this->load->view('temp/footer');
			$data['meta_title'] = lang('Exam name entry');
			$data['subview'] = 'add';
    ...
    public function all()
    {
		...
		$data['meta_title'] = lang('exam name list');
		$data['subview'] = 'all';
    	$this->load->view('backend/_layout_main', $data);
    }
