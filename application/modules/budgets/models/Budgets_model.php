...
class Budgets_model extends CI_Model {
    ...
    public function get_budget_chahida($limit, $offset, $arr = array(), $dept_id = null, $user_id = null, $type = null) {
        ...
        $this->db->join('session_year','q.fcl_year = session_year.id','left');
        $this->db->join('department dpt', 'q.dept_id = dpt.id', 'left');
        $this->db->join('users as u', 'u.id = q.created_by', 'left');
    ...
    public function get_budget_chahida_details($id) {
        $this->db->select('
                details.*,
                budget_head_sub.name_bn,
                budget_head.name_bn as budget_head_name,
            ');
        $this->db->from('budget_nilg_details as details');
        $this->db->join('budget_head_sub', 'details.head_sub_id = budget_head_sub.id');
        $this->db->join('budget_head', 'budget_head_sub.head_id = budget_head.id');
        $this->db->where('details.nilg_id', $id);
        $this->db->where('details.modify_soft_d', 1);
        // $this->db->get()->result();
        // dd($this->db->last_query());
        return $this->db->get()->result();
    }
    public function get_budget_chahida_info($id)
    {
        $this->db->select('
                nilg.*,
                dpt.name_en,
                ...
                btt.name as trainee_type,
            ');
        $this->db->from('budget_nilg as nilg');
        $this->db->join('department dpt', 'nilg.dept_id = dpt.id', 'left');
        $this->db->join('users as dptu', 'nilg.dpt_head_id = dptu.id', 'left');
        $this->db->join('users as acu', 'nilg.acc_head_id = acu.id', 'left');
        $this->db->join('users as crt', 'nilg.created_by = crt.id', 'left');
        $this->db->join('course', 'nilg.course_id = course.id', 'left');
        $this->db->join('budget_trainee_type btt', 'nilg.trainee_type = btt.id', 'left');
        $this->db->where('nilg.id', $id);
        return $this->db->get()->row();
    }
