					...
					<?php
								for($i=0;$i<sizeof($allcolumns);$i++)
								{
									...
									if(isset($divinfo[0][$fieldname]) and $dateclass!='')
									{
										...
										else $dtval=$divinfo[0][$fieldname];
										//$dtval=date('d-m-Y',strtotime($divinfo[0][$fieldname]));
									}
									else
										...
										<div class="form-group">
											<label class="form-label"><?php if(isset($get_lang_index[$fieldname]))echo $get_lang_index[$fieldname];else echo lang($fieldname);?></label><br>
													...
													<?php foreach ($arrydyn[2] as $value) { 
														...
														<input type="radio"  name="<?=($fieldname)?>" value="<?php echo $value[$arrydyn[1]] ?>" <?=$checkval;?>> <?php echo $value[$arrydyn[1]] ?>
													<?php } ?>
												</select>
										</div>
							<?php
							...
							<?php
									...
									else if($fieldname=='data_sheet_id' or $fieldname=='entry_date' or $fieldname=='applicant_name' or $fieldname=='birth_date')
									{
										// print_r($arrydyn);exit;
							?>
										<div class="form-group">
											...
											<?php echo form_error('first_name'); ?>
												<input type="text" readonly="readonly" class="form-control input-sm" data-validation="required" data-validation-error-msg="<?php echo lang($fieldname).' is required field.'; ?>" placeholder="" value="<?=$dtval?>" name="<?=$fieldname?>">
										</div>
							<?php
									}
									else
